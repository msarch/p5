...
class Ten {
  ...
  // is made 10x bigger and scaled  back to 0.1 for better aspect 
  // since pixel / divisions roundup leads to rectangles overlap
  ...
  Ten(int xc, int yc, float scale){}
    ...
    void display () {
      ...
      color[] colorRandomize() {
      // Get a random element from an array
      for (int i=0;i<7;i++){
          //int index = (int)(random(4));  // palette length
          currentPalette[i] = basePalette[2];  // Prints one of the four words
      }
